    ...
    <div class="py-8 bg-gray-50 dark:bg-gray-900/50 min-h-screen font-sans">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            @if(!empty($globalAnnouncement))
                <div class="bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-950/20 dark:to-orange-950/20 border-l-4 border-amber-500 p-6 rounded-r-2xl shadow-sm border border-amber-100 dark:border-amber-900 flex gap-4 items-start relative overflow-hidden">
                    ...
                    <div class="flex-grow">
                        <h4 class="text-sm font-extrabold text-amber-800 dark:text-amber-400 uppercase tracking-wider mb-1">Pengumuman Penting</h4>
                        <div class="text-sm text-amber-950 dark:text-amber-300 font-semibold leading-relaxed prose prose-amber max-w-none">
...
@endif
            ...
            @if(empty(Auth::user()->nik) || empty(Auth::user()->asal_instansi))
                <div class="bg-yellow-50 dark:bg-yellow-950/20 border-l-4 border-yellow-400 p-4 rounded-r-xl shadow-sm border border-yellow-100 dark:border-yellow-900/50">
                    <div class="flex">
                        <div class="flex-shrink-0">
                            <i class="fas fa-exclamation-circle text-yellow-500 text-xl"></i>
                        </div>
                        <div class="ml-3">
                            <h3 class="text-sm font-bold text-yellow-800 dark:text-yellow-400">Profil Belum Lengkap</h3>
                            <div class="mt-1 text-sm text-yellow-700 dark:text-yellow-400">
                                ...
                                <a href="{{ route('profile.edit') }}" class="font-bold underline hover:text-yellow-900 dark:hover:text-yellow-300">Lengkapi Sekarang &rarr;</a>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="p-6 md:p-8 flex flex-col md:flex-row justify-between items-center gap-6">
                        ...
                        <div class="flex flex-wrap justify-center gap-3 w-full md:w-auto">
                            @if($activeApp->display_status == 'selesai')
                                <div class="px-6 py-3 bg-blue-50 dark:bg-blue-950/30 text-blue-700 dark:text-blue-400 rounded-xl border border-blue-200 dark:border-blue-900 font-bold flex items-center gap-2 shadow-sm">
                                    <i class="fas fa-flag-checkered text-blue-500"></i> Magang Selesai
                                </div>
                            @elseif($activeApp->display_status == 'belum mulai')
                                <div class="px-6 py-3 bg-indigo-50 dark:bg-indigo-950/30 text-indigo-700 dark:text-indigo-400 rounded-xl border border-indigo-200 dark:border-indigo-900 font-bold flex items-center gap-2 shadow-sm">
                            ...
                            @elseif(!$attendanceToday)
                                <div class="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                                    ...
                                    <button x-data="" x-on:click.prevent="$dispatch('open-modal', 'modal-izin')" class="w-full sm:w-auto justify-center px-6 py-3.5 bg-white dark:bg-gray-800 border-2 border-yellow-400 text-yellow-600 hover:bg-yellow-50 dark:hover:bg-yellow-950/20 rounded-xl font-black transition active:scale-95 flex items-center gap-2 text-sm">
                             ...
                             @elseif($attendanceToday->status == 'hadir' && empty($attendanceToday->clock_out))
                                <div class="flex flex-col items-center gap-3 w-full sm:w-auto">
                                    <div class="text-xs font-bold text-teal-700 dark:text-teal-400 bg-teal-50 dark:bg-teal-950/30 px-4 py-2 rounded-lg border border-teal-200 dark:border-teal-900">
                            ...
                            @else
                                <div class="text-center">
                                    @if($attendanceToday->status == 'hadir')
                                        <div class="px-6 py-4 bg-green-50 dark:bg-green-950/30 text-green-700 dark:text-green-400 rounded-xl border border-green-200 dark:border-green-900 font-bold flex flex-col items-center shadow-sm">
                                            <span class="flex items-center gap-1"><i class="fas fa-check-double text-green-600"></i> Kehadiran Terekam</span>
                                            <span class="text-xs font-normal mt-1 text-green-600 dark:text-green-500">
                                    ...
                                    @else
                                        <div class="px-6 py-3 bg-yellow-50 dark:bg-yellow-950/30 text-yellow-700 dark:text-yellow-400 rounded-xl border border-yellow-200 dark:border-yellow-900 font-bold flex items-center gap-2 shadow-sm">
    ...
    function autoDetectGPS() {
        ...
        if (!navigator.geolocation) {
            ...
            title.className = "text-xs font-extrabold text-red-900 dark:text-red-300 uppercase tracking-wider";
            title.innerText = "GPS Tidak Didukung";
            desc.className = "text-xs text-red-600 dark:text-red-400 font-medium";
            desc.innerText = "Browser Anda tidak mendukung fitur geolokasi GPS.";
            badge.className = "px-3.5 py-1.5 rounded-xl bg-white dark:bg-gray-800 text-red-700 dark:text-red-400 text-xs font-extrabold shadow-sm border border-red-200/60 dark:border-red-900/50 flex items-center gap-1.5";
        ...
        navigator.geolocation.getCurrentPosition(
            function(position) {
                ...
                if (distance <= radius) {
                    ...
                    banner.className = "px-6 py-4 bg-orange-50/80 dark:bg-orange-950/20 border-t border-orange-200/60 dark:border-orange-900/50 flex items-center justify-between flex-wrap gap-3 transition-all duration-300";
                    ...
                    iconWrapper.innerHTML = '<i class="fas fa-exclamation-triangle"></i>';
                    title.className = "text-xs font-extrabold text-orange-900 dark:text-orange-300 uppercase tracking-wider";
                    title.innerText = "Di Luar Radius Kantor";
                    desc.className = "text-xs text-orange-700 dark:text-orange-400 font-medium";
                    desc.innerText = `Jarak Anda: ${distance} meter dari kantor (Batas maksimal ${radius}m). Mendekatlah ke kantor untuk absen.`;
                    badge.className = "px-3.5 py-1.5 rounded-xl bg-white dark:bg-gray-800 text-orange-700 dark:text-orange-400 text-xs font-extrabold shadow-sm border border-orange-200/60 dark:border-orange-900/50 flex items-center gap-1.5";
                    badge.innerHTML = `<i class="fas fa-ruler-horizontal text-orange-500"></i> Jarak: ${distance}m`;
                }
            },
            function(error) {
                banner.className = "px-6 py-4 bg-red-50/80 dark:bg-red-950/20 border-t border-red-200/60 dark:border-red-900/50 flex items-center justify-between flex-wrap gap-3 transition-all duration-300";
                iconWrapper.className = "w-10 h-10 rounded-xl bg-red-500 text-white flex items-center justify-center text-lg shadow-md shadow-red-500/20";
                ...
                desc.className = "text-xs text-red-700 dark:text-red-400 font-medium";
                desc.innerText = errorMsg;
                badge.className = "px-3.5 py-1.5 rounded-xl bg-white dark:bg-gray-800 text-red-700 dark:text-red-400 text-xs font-extrabold shadow-sm border border-red-200/60 dark:border-red-900/50 flex items-center gap-1.5 cursor-pointer hover:bg-red-50 dark:hover:bg-red-950/20";
