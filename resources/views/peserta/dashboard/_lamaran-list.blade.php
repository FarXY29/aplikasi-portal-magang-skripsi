                    ...
                    <form action="{{ route('peserta.dashboard') }}" method="GET" class="w-full md:w-auto flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center gap-2 sm:gap-3">
                        ...
                        <div class="flex items-center gap-2 w-full sm:w-auto justify-between sm:justify-start">
                            ...
                            @if(request('status') || request('start_date') || request('end_date'))
                                <a href="{{ route('peserta.dashboard') }}" class="min-w-[44px] min-h-[44px] flex items-center justify-center p-2 bg-rose-50 dark:bg-rose-950/60 text-rose-600 dark:text-rose-400 hover:bg-rose-100 border border-rose-200 dark:border-rose-800/60 rounded-xl transition shrink-0" title="Reset Filter">
                ...
                <div class="p-4 sm:p-6 space-y-4">
                        ...
                        <div x-data class="flex flex-col lg:flex-row justify-between items-start lg:items-center p-4 sm:p-5 rounded-2xl border transition hover:shadow-xs cursor-pointer gap-4 {{ $appStatus == 'diterima' ? 'bg-teal-50/40 dark:bg-teal-950/20 border-teal-200 dark:border-teal-900/60 hover:border-teal-400' : ($appStatus == 'selesai' ? 'bg-blue-50/40 dark:bg-blue-950/20 border-blue-200 dark:border-blue-900/60 hover:border-blue-400' : ($appStatus == 'menunggu' ? 'bg-amber-50/40 dark:bg-amber-950/20 border-amber-200 dark:border-amber-900/60' : 'bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 hover:border-teal-300 dark:hover:border-teal-700')) }}"
                            ...
                            <div class="w-full lg:flex-1 min-w-0">
                                <div class="flex flex-wrap items-center gap-2 mb-1.5">
                                    <h4 class="font-black text-gray-900 dark:text-gray-100 text-sm sm:text-base md:text-lg leading-tight">{{ $app->position->instansi->nama_dinas }}</h4>
                                    @php
                                        $badges = [
                                ...
                                <div class="flex flex-col sm:flex-row sm:flex-wrap items-start sm:items-center gap-x-4 gap-y-1 mt-2 text-xs text-gray-500 dark:text-gray-400 font-medium">
                                    ...
                                    <span class="flex items-center gap-1.5">
                                        ...
                                        Periode: <span class="text-gray-800 dark:text-gray-200 font-bold">{{ \Carbon\Carbon::parse($app->tanggal_mulai)->translatedFormat('d M Y') }} &mdash; {{ \Carbon\Carbon::parse($app->tanggal_selesai)->translatedFormat('d M Y') }}</span>
                                    </span>
                                    @endif
                                </div>
                    ...
                    @empty
                        <x-ui.empty-state 
                            ...
                            icon="fa-inbox">
                            @if(request('status') || request('start_date') || request('end_date'))
                                <a href="{{ route('peserta.dashboard') }}" class="min-h-[44px] inline-flex items-center px-4 py-2 bg-rose-50 dark:bg-rose-950/60 text-rose-700 dark:text-rose-300 hover:bg-rose-100 border border-rose-200 dark:border-rose-800/60 rounded-xl text-xs font-bold transition">Reset Filter</a>
                            @else
                                <a href="{{ route('home') }}" class="min-h-[44px] inline-flex items-center px-4 py-2.5 bg-teal-600 text-white hover:bg-teal-700 rounded-xl text-xs font-bold transition shadow-xs">Cari Lowongan Magang &rarr;</a>
                            @endif
                        </x-ui.empty-state>
                    @endforelse
                </div>
