            ...
            <div class="bg-white dark:bg-gray-800 p-5 rounded-3xl shadow-xs border border-gray-100 dark:border-gray-700 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <form action="{{ route('pembimbing_lapangan.logbook', $app->id) }}" method="GET" class="w-full md:w-auto flex flex-wrap items-center gap-4">
                    ...
                    @if(request('filter_type') && request('filter_type') != 'semua')
                        <a href="{{ route('pembimbing_lapangan.logbook', $app->id) }}" class="p-2 bg-rose-50 dark:bg-rose-950/60 text-rose-700 dark:text-rose-300 hover:bg-rose-100 border border-rose-200 dark:border-rose-800/60 rounded-xl transition text-xs font-bold flex items-center gap-1.5" title="Reset Filter">
