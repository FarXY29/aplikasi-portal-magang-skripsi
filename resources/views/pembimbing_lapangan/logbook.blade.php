    ...
    <div class="py-8 bg-gray-50/50 min-h-screen font-sans">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="flex justify-between items-center mb-6 print:hidden">
                <a href="{{ route('pembimbing_lapangan.dashboard') }}" class="group flex items-center text-sm font-bold text-gray-500 hover:text-teal-600 transition">
            ...
            <!-- Filter Bar -->
            <div class="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div class="flex items-center gap-2">
                    <i class="fas fa-filter text-teal-600"></i>
                    <span class="text-sm font-bold text-gray-700">Filter Logbook</span>
                </div>
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            @if($filterType === 'bulanan')
                                <input type="month" name="month" value="{{ \Carbon\Carbon::parse($selectedDate)->format('Y-m') }}" 
                                    class="border-gray-200 rounded-xl text-xs shadow-sm focus:border-teal-500 focus:ring-teal-500 bg-gray-50 text-gray-700 transition py-1.5 px-3"
                                    onchange="this.form.date.value = this.value + '-01'; this.form.submit();">
                                <input type="hidden" name="date" value="{{ $selectedDate }}">
                            @else
                                <input type="date" name="date" value="{{ $selectedDate }}" onchange="this.form.submit()" 
                                    class="border-gray-255 rounded-xl text-xs shadow-sm focus:border-teal-500 focus:ring-teal-500 bg-gray-50 text-gray-700 transition py-1.5 px-3">
                            @endif
                        </div>
                    @endif
            ...
            @if($logs->isEmpty())
                <div class="flex flex-col items-center justify-center py-16 bg-white rounded-2xl shadow-sm border border-gray-100">
                    ...
                    @if($filterType !== 'semua')
                        <p class="text-sm text-gray-500 mt-1">Tidak ada aktivitas logbook pada periode yang dipilih.</p>
                            ...
                            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                                ...
                                <div class="p-6">
                                    <div class="flex flex-col lg:flex-row gap-8">
                                        ...
                                        <div class="w-full lg:w-1/3 flex-shrink-0">
                                            ...
                                            @if($log->bukti_foto_path)
                                                <div class="relative group rounded-xl overflow-hidden shadow-sm border border-gray-200 cursor-zoom-in" onclick="openImageModal('{{ Storage::url($log->bukti_foto_path) }}')">
                                    ...
                                    <div class="mt-8 pt-6 border-t border-gray-100">
                                        ...
                                        <form action="{{ route('pembimbing_lapangan.logbook.validasi', $log->id) }}" method="POST" class="bg-gray-50 p-5 rounded-2xl border border-gray-200">
                                                @csrf
