     ...
     <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex-grow w-full">
         ...
         <div class="bg-gradient-to-r from-teal-900 via-teal-950 to-emerald-950 rounded-[2.5rem] p-6 sm:p-10 text-white shadow-xl shadow-teal-950/35 mb-12 overflow-hidden relative border border-teal-800/40 w-full">
             ...
             <div class="flex flex-col lg:flex-row gap-6 lg:gap-8 items-stretch lg:items-center justify-between relative z-10 w-full">
                 ...
                 <a href="{{ route('peserta.apply_automatic.form') }}" class="shrink-0 bg-white dark:bg-gray-800 text-teal-950 dark:text-white hover:bg-teal-50 dark:hover:bg-gray-700 px-6 py-4 rounded-2xl font-extrabold shadow-md hover:shadow-lg transition transform hover:-translate-y-0.5 active:scale-98 text-center text-sm flex items-center justify-center gap-2">
         ...
         <!-- Vacancies Section Header -->
          <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4 w-full">
              <div>
                  ...
                  <h2 id="lowongan" class="text-2xl sm:text-4xl font-extrabold text-slate-800 dark:text-white tracking-tight mt-4 scroll-mt-[95px]">Lowongan Magang Terbaru</h2>
                  <p class="text-slate-500 dark:text-slate-400 mt-2 text-sm sm:text-base">Dapatkan kesempatan berharga untuk mengabdi dan belajar langsung di instansi pemerintahan.</p>
              </div>
          ...
          <!-- Filter Dock Card -->
          <div class="bg-white dark:bg-gray-800 p-5 sm:p-8 rounded-[2.5rem] shadow-lg shadow-slate-200/40 dark:shadow-none border border-slate-100 dark:border-gray-700/50 mb-8 w-full">
                  ...
                  <div class="grid grid-cols-1 lg:grid-cols-12 gap-5 lg:gap-6 items-end w-full">
                      ...
                      <div class="lg:col-span-5 w-full">
                          ...
                          <div class="relative w-full group">
                              <select name="instansi_id" class="w-full pl-5 pr-12 py-4 border border-slate-200 dark:border-gray-700 rounded-2xl focus:ring-2 focus:ring-teal-500 focus:border-teal-500 text-base lg:text-sm font-semibold bg-slate-50/50 dark:bg-gray-900 focus:bg-white dark:focus:bg-gray-800 transition duration-300 appearance-none cursor-pointer text-slate-800 dark:text-slate-200 shadow-sm">
                      ...
                      <div class="lg:col-span-5 w-full">
                          ...
                          <div class="relative w-full">
                              <input type="text" name="jurusan" id="jurusan-input" value="{{ request('jurusan') }}" placeholder="Contoh: Informatika, Akuntansi, SMK..." 
                                  class="w-full px-5 py-4 border border-slate-200 dark:border-gray-700 rounded-2xl focus:ring-2 focus:ring-teal-500 focus:border-teal-500 text-base lg:text-sm font-semibold bg-slate-50/50 dark:bg-gray-900 focus:bg-white dark:focus:bg-gray-800 transition duration-300 text-slate-800 dark:text-slate-200 placeholder-slate-400 shadow-sm">
                          </div>
                      </div>

                      <!-- Filter Button -->
                      <div class="lg:col-span-2 w-full">
                          <button type="submit" class="w-full bg-slate-900 dark:bg-teal-600 hover:bg-teal-600 dark:hover:bg-teal-500 text-white font-extrabold py-4 px-6 rounded-2xl shadow-md hover:shadow-lg active:scale-98 transition-all duration-300 flex items-center justify-center gap-2 text-sm">
          ...
          <div class="flex items-center gap-3.5 overflow-x-auto pb-4 mb-8 no-scrollbar w-full max-w-full">
              <span class="text-xs font-extrabold text-slate-400 dark:text-slate-400 uppercase tracking-wider shrink-0 mr-1 flex items-center">
                  <i class="fas fa-bolt text-amber-500 mr-2"></i> Filter Cepat:
              </span>
              <a href="{{ route('home') }}#lowongan" class="shrink-0 px-4 py-2 rounded-full text-xs font-extrabold transition-all duration-300 {{ !request('jurusan') && !request('instansi_id') && !request('search') ? 'bg-gradient-to-r from-teal-600 to-emerald-600 text-white shadow-md shadow-teal-600/25' : 'bg-white dark:bg-gray-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-gray-700 hover:border-teal-300 dark:hover:border-teal-500 hover:text-teal-600 dark:hover:text-teal-400 shadow-sm' }}">
                  ✨ Semua Posisi
              </a>
             ...
             @forelse($lowongans as $loker)
                 @if($loker->kuota > 0)
                     <div x-data="{ showModal: false }" class="h-full flex flex-col w-full">
                         <!-- Job Card Wrapper -->
                         <div @click="showModal = true" class="cursor-pointer group bg-white dark:bg-gray-800 rounded-[2rem] border border-slate-200/80 dark:border-gray-700/80 overflow-hidden hover:shadow-xl hover:shadow-slate-100 dark:hover:shadow-none hover:border-teal-300 dark:hover:border-teal-500 hover:-translate-y-1 active:scale-[0.99] transition-all duration-300 flex flex-col h-full relative shadow-sm w-full">
                         
                             <!-- Card Header Bar -->
                             <div class="px-6 pt-6 pb-4 flex items-center justify-between gap-3 border-b border-slate-50 dark:border-gray-700/50 shrink-0">
                                 <div class="flex flex-wrap items-center gap-2">
                                     ...
                                     @else
                                         <span class="inline-flex items-center gap-1 bg-slate-100 dark:bg-gray-700 text-slate-600 dark:text-slate-300 text-[10px] px-2.5 py-1 rounded-lg font-bold">
                                             💺 Kuota: {{ $loker->kuota }}
                                         </span>
                                     @endif
                                 </div>
                             ...
                             <div class="p-6 pt-5 pb-4 flex items-start gap-4 shrink-0">
                                 ...
                                 <div class="min-w-0 flex-grow">
                                     <h3 class="text-base sm:text-lg font-black text-slate-800 dark:text-slate-100 group-hover:text-teal-600 dark:group-hover:text-teal-400 transition-colors duration-300 line-clamp-1 leading-snug" title="{{ $loker->judul_posisi }}">
                                         {{ $loker->judul_posisi }}
                                     </h3>
                             ...
                             <div class="px-6 py-2 flex-grow flex flex-col justify-start">
                                 <div class="flex flex-wrap items-center gap-2 mb-4">
                                     <span class="inline-flex items-center px-3 py-1 rounded-xl text-[11px] font-bold bg-slate-100 dark:bg-gray-700 text-slate-700 dark:text-slate-300 border border-slate-200/80 dark:border-gray-600/50 max-w-full">
                                     ...
                                     @if(preg_match('/SMA|SMK/i', $loker->required_major))
                                         <span class="inline-flex items-center px-2.5 py-1 rounded-xl text-[10px] font-extrabold bg-indigo-50 dark:bg-indigo-950/40 text-indigo-700 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-900/50 shrink-0">
                                             🎒 SMA/SMK
                                         </span>
                                     @endif
                                 </div>
                             ...
                             <div class="p-5 pt-3 bg-slate-50 dark:bg-gray-900/30 border-t border-slate-100 dark:border-gray-700/50 mt-auto shrink-0 flex items-center justify-between gap-3">
                                 @auth
                                     @if(auth()->user()->role == 'peserta')
                                         ...
                                         @else
                                             <button disabled class="w-full bg-slate-200 dark:bg-gray-800 text-slate-400 dark:text-slate-600 py-3.5 px-4 rounded-2xl font-bold cursor-not-allowed text-xs sm:text-sm flex items-center justify-center gap-2">
                                                 <i class="fas fa-lock text-xs"></i> Syarat Jurusan Tidak Sesuai
                                             </button>
                                         @endif
                                     @elseif(auth()->user()->role == 'admin_kota' || auth()->user()->role == 'admin_instansi')
                                         <button disabled class="w-full text-center text-xs font-extrabold text-slate-400 dark:text-slate-600 py-3 bg-slate-100 dark:bg-gray-800 border border-slate-200 dark:border-gray-700 rounded-xl">Pratinjau Admin</button>
                                     @endif
                                 @else
                                     <a @click.stop href="{{ route('peserta.daftar.form', $loker->id) }}" class="w-full bg-slate-900 dark:bg-gray-950 dark:text-slate-200 hover:bg-teal-600 dark:hover:bg-teal-600 text-white py-3.5 px-4 rounded-2xl font-extrabold shadow-md active:scale-98 transition-all duration-300 text-center text-xs sm:text-sm flex items-center justify-center gap-2">
                         ...
                         <div x-show="showModal" style="display: none;" class="fixed inset-0 z-[100] flex justify-end items-end sm:items-stretch" role="dialog" aria-modal="true">
                             ...
                             <div x-show="showModal" 
                                  ...
                                  class="relative bg-white dark:bg-gray-900 rounded-t-[2.5rem] sm:rounded-t-none sm:rounded-l-[2.5rem] shadow-2xl w-full sm:max-w-md md:max-w-lg lg:max-w-xl xl:max-w-2xl h-[88vh] sm:h-full overflow-hidden flex flex-col z-10 transition-all text-left border-l border-slate-100 dark:border-gray-800">
                                 
                                 <!-- Mobile Drag handle bar -->
                                 <div class="sm:hidden w-12 h-1.5 bg-slate-200 dark:bg-slate-800 rounded-full mx-auto my-3.5 shrink-0"></div>

                                 <!-- Header Drawer -->
                                 <div class="px-6 sm:px-8 py-5 border-b border-slate-100 dark:border-gray-800 bg-slate-50/50 dark:bg-gray-900/50 shrink-0 flex justify-between items-center">
                                     ...
                                     <button @click="showModal = false" class="text-slate-400 hover:text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-gray-800 rounded-xl w-8 h-8 flex items-center justify-center transition-all border border-slate-200/50 dark:border-gray-700/50 shrink-0">
                                 ...
                                 <div class="px-6 sm:px-8 pt-6 pb-8 overflow-y-auto flex-grow space-y-6">
                                     ...
                                     <div class="space-y-3">
                                         ...
                                         <div class="prose prose-sm dark:prose-invert max-w-none text-slate-600 dark:text-slate-400 bg-slate-50/50 dark:bg-gray-900/30 p-5 rounded-2xl border border-slate-100/60 dark:border-gray-800 text-xs sm:text-sm shadow-inner leading-relaxed">
                                             {!! $loker->deskripsi !!}
                                         </div>
                                     </div>
                                 ...
                                 <div class="px-6 sm:px-8 py-5 border-t border-slate-100 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-[0_-4px_25px_rgba(0,0,0,0.04)] dark:shadow-none flex items-center justify-end gap-4 pb-safe sm:pb-5 shrink-0 z-20">
                                     ...
                                     @auth
                                         ...
                                         @endif
                                     @else
                                         <a href="{{ route('peserta.daftar.form', $loker->id) }}" class="bg-slate-900 dark:bg-gray-900 dark:text-slate-200 hover:bg-teal-600 dark:hover:bg-teal-600 text-white px-6 py-3.5 rounded-2xl font-extrabold shadow-md active:scale-98 transition-all duration-300 text-xs sm:text-sm flex items-center gap-2">
