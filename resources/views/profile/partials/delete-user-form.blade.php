    ...
    <x-modal name="confirm-user-deletion" :show="$errors->userDeletion->isNotEmpty()" focusable>
        <form method="post" action="{{ route('profile.destroy') }}" class="p-6 sm:p-7">
            ...
            <div class="space-y-1.5">
                <label class="flex items-center gap-1.5 text-sm leading-none font-medium text-gray-700 select-none" for="delete_password">
                ...
                <input
                    ...
                    name="password"
                    type="password"
                    class="w-full min-w-0 rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 shadow-2xs transition-all outline-none focus:border-red-600 focus:ring-3 focus:ring-red-600/15"
            ...
            <div class="mt-7 pt-5 border-t border-gray-100 flex items-center justify-end gap-3">
                <button type="button" x-on:click="$dispatch('close')" class="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-lg text-sm font-medium transition-all border border-gray-300 bg-white shadow-2xs hover:bg-gray-50 text-gray-700 h-9 px-4 py-2 cursor-pointer">
                    {{ __('Batal') }}
                </button>
