    ...
    <div class="py-8 bg-gray-50/50 min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="flex justify-between items-center mb-6">
                <a href="{{ route('pembimbing.dashboard') }}" class="group flex items-center text-sm font-bold text-gray-500 hover:text-teal-600 transition">
            ...
            <!-- Filter Bar -->
            <div class="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                <div class="flex items-center gap-2">
                    <i class="fas fa-filter text-teal-600"></i>
                    <span class="text-sm font-bold text-gray-700">Filter Absensi</span>
                </div>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-2xl border border-gray-100">
                <div class="p-6 border-b border-gray-50 flex justify-between items-center">
                    <h3 class="text-lg font-bold text-gray-800"><i class="fas fa-calendar-check text-teal-600 mr-2"></i> Rekap Kehadiran</h3>
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gray-50">
                                    <tr>
                                        ...
                                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Catatan/Keterangan</th>
                                    </tr>
                                </thead>
                                    ...
                                    @foreach($attendances as $absen)
                                        <tr class="hover:bg-gray-50 transition">
                                            ...
                                            <td class="px-6 py-4 text-center whitespace-nowrap">
                                                @if($absen->clock_in)
                                                    <span class="text-sm font-medium text-gray-900">{{ \Carbon\Carbon::parse($absen->clock_in)->format('H:i') }}</span>
                                                @else
                                            ...
                                            <td class="px-6 py-4 text-center whitespace-nowrap">
                                                @if($absen->clock_out)
                                                    <span class="text-sm font-medium text-gray-900">{{ \Carbon\Carbon::parse($absen->clock_out)->format('H:i') }}</span>
                                                @else
                                                    <span class="text-sm text-gray-400">-</span>
                                                @endif
                                            </td>
                                            ...
                                            <td class="px-6 py-4">
                                                <div class="text-sm text-gray-900 truncate max-w-xs" title="{{ $absen->description }}">
                                                    {{ $absen->description ?: '-' }}
                                                </div>
