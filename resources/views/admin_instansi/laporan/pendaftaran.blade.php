<x-app-layout>
    ...
    class="py-8 bg-slate-50 dark:bg-[#0f172a] min-h-screen font-sans text-slate-900 dark:text-slate-100">
        ...
        @php
            ...
            $buildAppItem = function ($app) {
                ...
                return [
                    ...
                    'phone' => $app->user->phone ?? ($app->user->phone_number ?? ($app->user->nomor_telepon ?? '-')),
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 print:hidden max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <a href="{{ route('dinas.laporan.hub') }}" class="group flex items-center text-sm font-bold text-slate-500 dark:text-slate-400 hover:text-teal-600 dark:hover:text-teal-400 transition">
            ...
            @if($applications->count() > 0)
                <a href="{{ route('dinas.laporan.pendaftaran.print', request()->query()) }}" target="_blank"
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4">
                {{-- Total Pendaftar --}}
                <div class="bg-white dark:bg-slate-800/90 rounded-2xl p-4 shadow-2xs border border-slate-200 dark:border-slate-700/80 text-center cursor-help transition hover:shadow-md" title="Total permohonan pendaftaran magang yang terdata di instansi Anda.">
                ...
                {{-- Status Pending --}}
                <div class="bg-white dark:bg-slate-800/90 rounded-2xl p-4 shadow-2xs border border-slate-200 dark:border-slate-700/80 text-center cursor-help transition hover:shadow-md" title="Permohonan baru yang masih menantikan proses peninjauan / verifikasi awal.">
            ...
            <div class="bg-gradient-to-r from-teal-700 via-teal-600 to-emerald-700 rounded-3xl p-5 sm:p-6 text-white shadow-lg shadow-teal-700/20 flex flex-col sm:flex-row items-center gap-4">
                ...
                <div class="text-center sm:text-left flex-grow">
                    <p class="text-xs font-bold uppercase tracking-wider text-teal-100">Pelacakan Status Real-Time di Instansi</p>
                    <p class="text-lg sm:text-xl font-black mt-0.5">Total {{ number_format($stats['total']) }} Berkas Permohonan Terdata</p>
                    <p class="text-xs sm:text-sm text-teal-100 font-medium">Monitoring pergerakan status pendaftaran mahasiswa dan siswa magang di lingkungan dinas/instansi terkait.</p>
                </div>
                @if($applications->count() > 0)
            ...
            <div class="bg-white dark:bg-slate-800/90 rounded-2xl sm:rounded-3xl p-5 sm:p-6 shadow-2xs border border-slate-200 dark:border-slate-700/80">
                <form method="GET" action="{{ route('dinas.laporan.pendaftaran') }}" class="space-y-4">
                    ...
                    {{-- Baris 2: Pencarian Cepat & Tombol Aksi --}}
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end pt-1">
                        {{-- Kolom Pencarian Cepat Server --}}
                        <div class="sm:col-span-2">
                            <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase mb-1.5">
                                <i class="fas fa-search text-teal-500 mr-1"></i> Pencarian Cepat Server
                            </label>
                            <div class="relative">
                        ...
                        <div class="flex items-center gap-2">
                            @if(request()->anyFilled(['status', 'posisi_id', 'start_date', 'end_date', 'search']))
                                <a href="{{ route('dinas.laporan.pendaftaran') }}" title="Bersihkan Filter"
            ...
            <div class="bg-white dark:bg-slate-800/90 rounded-2xl sm:rounded-3xl shadow-2xs border border-slate-200 dark:border-slate-700/80 overflow-hidden">
                <div class="p-5 sm:p-6 border-b border-slate-100 dark:border-slate-700/60 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 bg-slate-50/60 dark:bg-slate-900/40">
                    <div>
                        ...
                        <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5 font-medium">Rekapitulasi riwayat pengajuan, nomor registrasi, status verifikasi instansi, dan masa magang.</p>
                    </div>
                    ...
                    <table class="w-full divide-y divide-slate-100 dark:divide-slate-700">
                        <thead class="bg-slate-50 dark:bg-slate-900/80">
                            <tr>
                                ...
                                <th class="px-4 py-3.5 text-left text-xs font-black text-slate-500 dark:text-slate-400 uppercase tracking-wider whitespace-nowrap min-w-[200px]">Posisi / Formasi Magang</th>
                        ...
                        <tbody class="bg-white dark:bg-slate-800/90 divide-y divide-slate-100 dark:divide-slate-700/60 text-sm">
                            ...
                            <tr x-show="matchesSearch({{ json_encode($appItemJson) }})" 
                                ...
                                {{-- Posisi yang Dilamar & Pembimbing Lapangan --}}
                                <td class="px-4 py-3.5">
                            ...
                            <tr>
                                <td colspan="7" class="px-6 py-16 text-center">
                                    <div class="flex flex-col items-center justify-center text-slate-400 dark:text-slate-500">
                                        ...
                                        <p class="text-slate-500 dark:text-slate-400 text-sm mt-1">Coba sesuaikan kriteria filter status, posisi, atau kata kunci pencarian Anda.</p>
                                        <a href="{{ route('dinas.laporan.pendaftaran') }}" class="mt-4 text-teal-600 dark:text-teal-400 hover:underline text-sm font-bold">
                                            Reset Semua Filter
                                        </a>
                                    </div>
                                </td>
            ...
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div x-show="activeApp !== null" 
                    ...
                    <div class="p-6 space-y-6 max-h-[75vh] overflow-y-auto custom-scrollbar" x-if="activeApp">
                        ...
                        <div>
                            <h4 class="text-xs font-black uppercase tracking-wider text-slate-400 dark:text-slate-400 mb-3 flex items-center gap-1.5">
                                <i class="fas fa-building text-teal-500"></i> Formasi & Pembimbing Lapangan
                            </h4>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 bg-slate-50 dark:bg-slate-900/60 p-4 rounded-2xl border border-slate-200 dark:border-slate-700/60 text-xs">
                                <div>
