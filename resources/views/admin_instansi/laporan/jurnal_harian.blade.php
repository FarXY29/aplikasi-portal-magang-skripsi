        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                <div class="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 text-center">
                    ...
                    <p class="text-2xl font-black text-gray-800">{{ $stats['total_jurnal'] }}</p>
                    <p class="text-[10px] font-bold text-gray-500 uppercase tracking-wider mt-1">Total Jurnal</p>
                </div>
                <div class="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 text-center">
                    ...
                    <p class="text-[10px] font-bold text-gray-500 uppercase tracking-wider mt-1">Disetujui</p>
                </div>
                <div class="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 text-center">
                ...
                <div class="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 text-center">
                    ...
                    <p class="text-[10px] font-bold text-gray-500 uppercase tracking-wider mt-1">Revisi</p>
                </div>
                <div class="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 text-center">
                    ...
                    <p class="text-2xl font-black text-gray-800">{{ $stats['total_peserta_aktif'] }}</p>
                ...
                <div class="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 text-center">
                    ...
                    <p class="text-[10px] font-bold text-gray-500 uppercase tracking-wider mt-1">Rasio Validasi</p>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-r from-purple-600 to-indigo-600 rounded-2xl p-6 text-white shadow-lg shadow-purple-600/20 flex flex-col sm:flex-row items-center gap-4">
                ...
                <div class="text-center sm:text-left flex-grow">
                    ...
                    <p class="text-sm text-purple-100 font-medium">
                        Ada <span class="font-bold text-white bg-white/20 px-2.5 py-0.5 rounded">{{ $stats['pending'] }} jurnal</span> menunggu validasi. 
                ...
                <div class="flex flex-col sm:flex-row items-center gap-3 sm:ml-auto">
                    <form method="GET" action="{{ route('dinas.laporan.jurnal_harian') }}" class="flex-shrink-0">
                        <select name="filter" onchange="this.form.submit()" class="bg-purple-700/50 border border-purple-400 text-white rounded-xl text-xs font-bold focus:ring-purple-300 focus:border-purple-300 block w-full p-2.5 transition cursor-pointer">
                            <option value="semua" {{ $filter == 'semua' ? 'selected' : '' }} class="text-gray-800">Semua Waktu</option>
                            <option value="hari" {{ $filter == 'hari' ? 'selected' : '' }} class="text-gray-800">Hari Ini</option>
                            <option value="minggu" {{ $filter == 'minggu' ? 'selected' : '' }} class="text-gray-800">Minggu Ini</option>
                            <option value="bulan" {{ $filter == 'bulan' ? 'selected' : '' }} class="text-gray-800">Bulan Ini</option>
                        </select>
                    </form>

                    @if($jurnal->count() > 0)
                        <div class="flex gap-2">
                            <a href="{{ route('dinas.laporan.jurnal_harian.print', array_merge(['filter' => $filter], ['format' => 'pdf'])) }}" target="_blank" class="inline-flex items-center px-4 py-2.5 bg-white text-teal-700 rounded-xl hover:bg-teal-50 transition text-sm font-bold shadow-md border border-white/20" title="Download PDF">
                                <i class="fas fa-file-pdf text-red-500"></i> PDF
                            </a>
                            <a href="{{ route('dinas.laporan.jurnal_harian.print', array_merge(['filter' => $filter], ['format' => 'excel'])) }}" class="inline-flex items-center px-4 py-2.5 bg-white text-teal-700 rounded-xl hover:bg-teal-50 transition text-sm font-bold shadow-md border border-white/20" title="Download Excel">
                                <i class="fas fa-file-excel text-green-600"></i> Excel
                            </a>
                            <a href="{{ route('dinas.laporan.jurnal_harian.print', array_merge(['filter' => $filter], ['format' => 'csv'])) }}" class="inline-flex items-center px-4 py-2.5 bg-white text-teal-700 rounded-xl hover:bg-teal-50 transition text-sm font-bold shadow-md border border-white/20" title="Download CSV">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="p-6 border-b border-gray-100">
                    ...
                    <p class="text-xs text-gray-500 mt-1">Uraian kegiatan harian beserta status persetujuan dari masing-masing pembimbing lapangan dinas.</p>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-100">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-5 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider w-28">Tanggal</th>
                                <th class="px-5 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Mahasiswa & Posisi</th>
                                <th class="px-5 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Uraian Kegiatan</th>
                                <th class="px-5 py-4 text-center text-xs font-bold text-gray-500 uppercase tracking-wider w-20">Bukti</th>
                                <th class="px-5 py-4 text-center text-xs font-bold text-gray-500 uppercase tracking-wider w-36">Status Validasi</th>
                                <th class="px-5 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Pembimbing & Komentar</th>
                            </tr>
                        </thead>
                            ...
                            <tr class="hover:bg-purple-50/10 transition duration-150">
                                ...
                                <td class="px-5 py-4">
                                    <div class="flex items-center gap-2.5">
                                        ...
                                        <div class="min-w-0">
                                            <p class="text-xs font-black text-gray-900 truncate">{{ $log->application->user->name ?? 'User Terhapus' }}</p>
                                            <p class="text-[10px] text-gray-500 font-semibold truncate">{{ $log->application->position->judul_posisi ?? 'Posisi Terhapus' }}</p>
                                        </div>
                                    </div>
                                </td>

                                <td class="px-5 py-4">
                                    <div class="text-xs text-gray-700 whitespace-pre-wrap break-words max-w-xs md:max-w-sm">{{ $log->kegiatan }}</div>
                                </td>
                                ...
                                <td class="px-5 py-4 text-center">
                                    @if($log->bukti_foto_path)
                                        <a href="{{ asset('storage/' . $log->bukti_foto_path) }}" target="_blank" class="inline-block relative group" title="Lihat Bukti Foto">
                                            <img src="{{ asset('storage/' . $log->bukti_foto_path) }}" class="w-10 h-10 rounded-lg object-cover border border-gray-200 hover:scale-110 transition shadow-sm">
                                    ...
                                    @else
                                        <span class="inline-block p-1 bg-gray-50 border border-gray-150 rounded text-gray-400" title="Tidak ada bukti foto">
                                ...
                                <td class="px-5 py-4">
                                    @if($log->application->pembimbing_lapangan)
                                        <div class="text-xs font-bold text-gray-800">{{ $log->application->pembimbing_lapangan->name }}</div>
                                        @if($log->komentar_pembimbing_lapangan)
                                            <div class="mt-1 bg-gray-50 text-[10px] text-gray-600 p-2 rounded-lg border border-gray-150 italic relative max-w-xs">
                            ...
                            <tr>
                                <td colspan="7" class="px-6 py-16 text-center">
                                    <div class="flex flex-col items-center text-gray-400">
                                        ...
                                        <p class="font-bold text-gray-500">Belum ada data jurnal harian</p>
