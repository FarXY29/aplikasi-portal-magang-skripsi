<x-app-layout>
    ...
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="font-black text-xl text-gray-900 dark:text-gray-100 leading-tight">Edit Pengumuman</h2>
                <p class="text-xs text-gray-500 dark:text-gray-400 font-medium hidden sm:block">Perbarui isi konten, sasaran audiens, atau batas waktu penayangan</p>
            </div>
        </div>
    </x-slot>
            ...
            <form action="{{ route('admin.announcements.update', $announcement->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <select name="type" required class="w-full px-4 py-3 text-sm font-bold rounded-2xl border border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 focus:bg-white focus:ring-2 focus:ring-teal-500">
                            <option value="info" {{ old('type', $announcement->type) == 'info' ? 'selected' : '' }}>ℹ️ Info Umum (Biasa)</option>
                            <option value="urgent" {{ old('type', $announcement->type) == 'urgent' ? 'selected' : '' }}>🚨 Mendesak / Urgent (Merah)</option>
                            <option value="warning" {{ old('type', $announcement->type) == 'warning' ? 'selected' : '' }}>⚠️ Peringatan / Warning (Kuning)</option>
                            <option value="event" {{ old('type', $announcement->type) == 'event' ? 'selected' : '' }}>📅 Agenda Kegiatan / Event (Hijau)</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="target_audience" required class="w-full px-4 py-3 text-sm font-bold rounded-2xl border border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 focus:bg-white focus:ring-2 focus:ring-teal-500">
                            <option value="all" {{ old('target_audience', $announcement->target_audience) == 'all' ? 'selected' : '' }}>🌐 Seluruh Pengguna Portal</option>
                            <option value="peserta" {{ old('target_audience', $announcement->target_audience) == 'peserta' ? 'selected' : '' }}>🎓 Khusus Peserta Magang</option>
                            <option value="admin_instansi" {{ old('target_audience', $announcement->target_audience) == 'admin_instansi' ? 'selected' : '' }}>🏢 Khusus Admin Instansi / OPD</option>
                            <option value="pembimbing" {{ old('target_audience', $announcement->target_audience) == 'pembimbing' ? 'selected' : '' }}>👨‍🏫 Khusus Dosen & Pembimbing Lapangan</option>
                        </select>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea name="content" rows="7" required
                        class="w-full px-4 py-3 text-sm rounded-2xl border border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 focus:bg-white focus:ring-2 focus:ring-teal-500 leading-relaxed">{{ old('content', $announcement->content) }}</textarea>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                        <label class="block text-xs font-black text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-2">
                            Ganti Banner / Lampiran Gambar (Opsional)
                        </label>
                        @if($announcement->banner_image)
                            <div class="mb-2 flex items-center gap-2">
                                <img src="{{ asset('storage/' . $announcement->banner_image) }}" alt="Current Banner" class="w-16 h-12 object-cover rounded-xl border border-gray-200">
                                <span class="text-[11px] text-gray-400">Banner saat ini terpasang</span>
                            </div>
                        @endif
                ...
                <!-- Status Publikasi -->
                <div class="bg-gray-50 dark:bg-gray-900/50 p-5 rounded-2xl border border-gray-100 dark:border-gray-700/60 flex items-center justify-between">
                    <div>
                        <span class="text-sm font-bold text-gray-900 dark:text-gray-100">Status Penayangan</span>
                ...
                <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-100 dark:border-gray-700">
                    ...
                    <button type="submit" class="px-6 py-3 bg-teal-600 hover:bg-teal-700 text-white rounded-xl text-xs font-bold transition shadow-md flex items-center gap-2">
                        <i class="fas fa-save text-xs"></i> Perbarui Pengumuman
                    </button>
                </div>
            </form>
        </div>
