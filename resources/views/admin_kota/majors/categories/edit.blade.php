<x-app-layout>
    ...
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="font-black text-xl text-gray-900 dark:text-gray-100 leading-tight">Edit Rumpun Keilmuan</h2>
                <p class="text-xs text-gray-500 dark:text-gray-400 font-medium">Perbarui nama, kode singkatan, atau deskripsi rumpun</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="max-w-3xl mx-auto space-y-6 font-[Inter]">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 md:p-8">
            <form action="{{ route('admin.master.major-categories.update', $category->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name', $category->name) }}" required placeholder="Contoh: Teknologi Informasi & Rekayasa Komputer" class="w-full text-sm rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-100 placeholder-gray-400 focus:border-teal-500 focus:ring-teal-500 py-2.5 px-3 shadow-xs">
                    @error('name')
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3" placeholder="Penjelasan singkat bidang ilmu dan keahlian yang tercakup dalam rumpun ini..." class="w-full text-sm rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-100 placeholder-gray-400 focus:border-teal-500 focus:ring-teal-500 py-2.5 px-3 shadow-xs">{{ old('description', $category->description) }}</textarea>
                    @error('description')
                        <p class="text-xs text-red-500 mt-1 font-medium">{{ $message }}</p>
                    @enderror
                </div>
