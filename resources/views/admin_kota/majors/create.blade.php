<x-app-layout>
    ...
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="font-black text-xl text-gray-900 dark:text-gray-100 leading-tight">Tambah Program Studi / Jurusan</h2>
                <p class="text-xs text-gray-500 dark:text-gray-400 font-medium">Tambahkan standar nama jurusan dan rumpun keilmuan ke database master</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="max-w-3xl mx-auto space-y-6 font-[Inter]" x-data="{
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 md:p-8">
            <form action="{{ route('admin.master.majors.store') }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <select id="major_category_id" name="major_category_id" required class="w-full text-sm rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-100 focus:border-teal-500 focus:ring-teal-500 py-2.5 px-3 shadow-xs">
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" {{ old('major_category_id') == $category->id ? 'selected' : '' }}>
                    ...
                    <div class="grid grid-cols-3 sm:grid-cols-5 gap-3">
                        @foreach(['SMK', 'D3', 'D4', 'S1', 'S2'] as $degree)
                            <label class="flex items-center justify-center p-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900/50 cursor-pointer hover:border-teal-500 transition has-[:checked]:bg-teal-50 dark:has-[:checked]:bg-teal-950/40 has-[:checked]:border-teal-500 has-[:checked]:text-teal-700 dark:has-[:checked]:text-teal-400 font-bold text-xs">
                                <input type="radio" name="degree_level" value="{{ $degree }}" {{ old('degree_level', 'S1') == $degree ? 'checked' : '' }} class="sr-only">
                                <span>{{ $degree }}</span>
                            </label>
                        @endforeach
                    </div>
                ...
                <div class="pt-2">
                    <label class="flex items-center gap-3 cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', '1') == '1' ? 'checked' : '' }} class="w-4 h-4 text-teal-600 rounded border-gray-300 focus:ring-teal-500">
                        <div>
                            <span class="text-sm font-bold text-gray-800 dark:text-gray-200">Status Aktif</span>
                            <p class="text-xs text-gray-500 dark:text-gray-400">Jurusan aktif akan langsung muncul pada pilihan pendaftaran peserta magang.</p>
                        </div>
                    </label>
                </div>
                ...
                <div class="pt-6 border-t border-gray-100 dark:border-gray-700 flex justify-end gap-3">
                    ...
                    <button type="submit" class="px-6 py-2.5 bg-teal-600 hover:bg-teal-700 text-white rounded-xl font-bold text-xs uppercase tracking-wider transition shadow-sm">
                        <i class="fas fa-save mr-1.5"></i> Simpan Jurusan
                    </button>
                </div>
            </form>
        </div>
