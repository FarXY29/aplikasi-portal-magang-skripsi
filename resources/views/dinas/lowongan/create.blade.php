                    ...
                    <form action="{{ route('dinas.lowongan.store') }}" method="POST">
                        ...
                        <div class="space-y-8">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                                ...
                                <div class="relative group">
                                    <label class="block text-sm font-bold text-slate-700 mb-2 flex items-center gap-2">
                                        <i class="fas fa-graduation-cap text-teal-500"></i> Syarat Jurusan
                                    </label>
                                    ...
                                    @error('required_major') <span class="text-red-500 text-xs mt-1 font-bold animate-pulse">{{ $message }}</span> @enderror
                                </div>
                                ...
                                <div class="relative group">
                                    ...
                                    <div class="relative transition-all duration-300 group-focus-within:drop-shadow-md">
                                        <input type="number" name="kuota" value="{{ old('kuota', 1) }}" min="1" 
                                            class="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl focus:bg-white focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 transition-all font-medium text-slate-800" required>
                                    </div>
                                    @error('kuota') <span class="text-red-500 text-xs mt-1 font-bold animate-pulse">{{ $message }}</span> @enderror
                                </div>
                            ...
                            <div class="relative group">
                                ...
                                <div class="relative transition-all duration-300 group-focus-within:drop-shadow-md md:w-1/2">
                                    <input type="date" name="batas_daftar" value="{{ old('batas_daftar') }}" 
                                        class="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl focus:bg-white focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition-all font-medium text-slate-800" required>
                                </div>
                                @error('batas_daftar') <span class="text-red-500 text-xs mt-1 font-bold animate-pulse">{{ $message }}</span> @enderror
                            </div>
                            ...
                            <div class="relative group">
                                ...
                                @error('deskripsi') <span class="text-red-500 text-xs mt-1 font-bold animate-pulse">{{ $message }}</span> @enderror
                            </div>
