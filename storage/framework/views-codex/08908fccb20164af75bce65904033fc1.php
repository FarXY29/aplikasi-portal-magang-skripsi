...
<?php $component->withAttributes([]); ?>
     <?php $__env->slot('header', null, []); ?> 
        <div class="flex flex-col md:flex-row justify-between items-center gap-4">
            <h2 class="font-extrabold text-2xl text-gray-800 dark:text-gray-200 leading-tight flex items-center gap-3">
                ...
                <?php echo e(__('Laporan Jurnal / Aktivitas Harian')); ?>
    ...
    <div class="py-8 bg-gray-50 dark:bg-gray-900 min-h-screen font-sans">
        <div class="flex justify-between items-center mb-6 print:hidden max-w-7xl mx-auto sm:px-6 lg:px-8">
            <a href="<?php echo e(route('dinas.laporan.hub')); ?>" class="group flex items-center text-sm font-bold text-gray-500 dark:text-gray-400 hover:text-purple-600 dark:hover:text-purple-400 transition">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                <div class="bg-white dark:bg-gray-800 rounded-3xl p-4 shadow-xs border border-gray-100 dark:border-gray-700 text-center cursor-help transition hover:shadow-md" title="Total seluruh akumulasi entri laporan kegiatan harian (logbook) yang diinput oleh peserta magang pada periode terpilih.">
                    ...
                    <p class="text-2xl font-black text-gray-800 dark:text-gray-100"><?php echo e($stats['total_jurnal']); ?></p>
                    <p class="text-[9px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-wider mt-1">Total Jurnal</p>
                </div>
                <div class="bg-white dark:bg-gray-800 rounded-3xl p-4 shadow-xs border border-gray-100 dark:border-gray-700 text-center cursor-help transition hover:shadow-md" title="Jumlah entri jurnal harian yang telah diverifikasi dan disetujui oleh pembimbing lapangan.">
                    ...
                    <p class="text-2xl font-black text-green-700 dark:text-green-400"><?php echo e($stats['disetujui']); ?></p>
                    <p class="text-[9px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-wider mt-1">Disetujui</p>
                </div>
                <div class="bg-white dark:bg-gray-800 rounded-3xl p-4 shadow-xs border border-gray-100 dark:border-gray-700 text-center cursor-help transition hover:shadow-md" title="Jumlah entri jurnal harian yang belum diverifikasi dan masih menantikan validasi pembimbing.">
                    ...
                    <p class="text-2xl font-black text-amber-600 dark:text-amber-400"><?php echo e($stats['pending']); ?></p>
                    <p class="text-[9px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-wider mt-1">Pending</p>
                </div>
                <div class="bg-white dark:bg-gray-800 rounded-3xl p-4 shadow-xs border border-gray-100 dark:border-gray-700 text-center cursor-help transition hover:shadow-md" title="Jumlah entri jurnal harian yang dikembalikan kepada peserta oleh pembimbing untuk diperbaiki catatannya.">
                    ...
                    <p class="text-2xl font-black text-red-700 dark:text-red-400"><?php echo e($stats['revisi']); ?></p>
                    <p class="text-[9px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-wider mt-1">Revisi</p>
                </div>
                <div class="bg-white dark:bg-gray-800 rounded-3xl p-4 shadow-xs border border-gray-100 dark:border-gray-700 text-center cursor-help transition hover:shadow-md" title="Jumlah peserta magang aktif yang aktif menginput entri laporan jurnal harian pada periode terpilih.">
                    ...
                    <p class="text-2xl font-black text-gray-800 dark:text-gray-100"><?php echo e($stats['total_peserta_aktif']); ?></p>
                    <p class="text-[9px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-wider mt-1">Peserta Aktif</p>
                </div>
                <div class="bg-white dark:bg-gray-800 rounded-3xl p-4 shadow-xs border border-gray-100 dark:border-gray-700 text-center cursor-help transition hover:shadow-md" title="Persentase kelengkapan validasi jurnal harian. Rumus: (Jumlah Jurnal Disetujui / Total Jurnal) x 100%.">
                    ...
                    <p class="text-2xl font-black text-indigo-700 dark:text-indigo-400"><?php echo e($stats['rasio_validasi']); ?>%</p>
                    <p class="text-[9px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-wider mt-1">Rasio Validasi</p>
                </div>
            </div>

            
            <div class="bg-gradient-to-r from-purple-600 to-indigo-600 rounded-3xl p-6 text-white shadow-lg shadow-purple-600/20 flex flex-col sm:flex-row items-center gap-4 cursor-help" title="Ringkasan statistik penuntasan validasi jurnal harian dan sisa jurnal pending sesuai periode terpilih.">
                ...
                <div class="text-center sm:text-left flex-grow">
                    <p class="text-xs font-bold uppercase tracking-wider text-purple-100">Rekapitulasi Jurnal Periode: <span class="underline font-bold text-white"><?php echo e($filter == 'semua' ? 'Semua Waktu' : ($filter == 'hari' ? 'Hari Ini' : ($filter == 'minggu' ? 'Minggu Ini' : 'Bulan Ini'))); ?></span></p>
                    <p class="text-xl font-black mt-0.5">Terdapat <?php echo e($stats['total_jurnal']); ?> Entri Jurnal</p>
                    <p class="text-sm text-purple-100 font-medium">
                        Ada <span class="font-bold text-white bg-white/20 dark:bg-gray-800/30 px-2.5 py-0.5 rounded-lg border border-white/20"><?php echo e($stats['pending']); ?> jurnal</span> menunggu validasi. 
                        Rasio jurnal tuntas disetujui mencapai <span class="font-bold text-white text-md"><?php echo e($stats['rasio_validasi']); ?>%</span>.
                    </p>
                </div>
                <div class="flex flex-col sm:flex-row items-center gap-3 sm:ml-auto">
                    <form method="GET" action="<?php echo e(route('dinas.laporan.jurnal_harian')); ?>" class="flex-shrink-0">
                        <select name="filter" onchange="this.form.submit()" class="bg-purple-700/60 dark:bg-gray-800 border border-purple-400/80 dark:border-gray-700 text-white rounded-xl text-xs font-bold focus:ring-purple-300 focus:border-purple-300 block w-full p-2.5 transition cursor-pointer">
                            <option value="semua" <?php echo e($filter == 'semua' ? 'selected' : ''); ?> class="bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-100">Semua Waktu</option>
                            <option value="hari" <?php echo e($filter == 'hari' ? 'selected' : ''); ?> class="bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-100">Hari Ini</option>
                            <option value="minggu" <?php echo e($filter == 'minggu' ? 'selected' : ''); ?> class="bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-100">Minggu Ini</option>
                            <option value="bulan" <?php echo e($filter == 'bulan' ? 'selected' : ''); ?> class="bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-100">Bulan Ini</option>
                        </select>
                    </form>

                    <?php if($jurnal->count() > 0): ?>
                        <div class="flex gap-2">
                            <a href="<?php echo e(route('dinas.laporan.jurnal_harian.print', ['filter' => $filter])); ?>" target="_blank" class="inline-flex items-center px-4 py-2.5 bg-white dark:bg-gray-800 text-purple-700 dark:text-purple-400 hover:text-purple-800 dark:hover:text-purple-300 rounded-xl hover:bg-purple-50 dark:hover:bg-gray-700 transition text-xs font-bold shadow-md border border-white/20 dark:border-gray-700" title="Download PDF">
                    ...
                    <table class="w-full divide-y divide-gray-100 dark:divide-gray-700">
                        ...
                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-100 dark:divide-gray-700/60 text-sm">
                            ...
                            <tr class="hover:bg-purple-50/15 dark:hover:bg-purple-950/20 transition duration-150">
                                ...
                                <td class="px-5 py-4">
                                    <div class="text-sm font-bold text-gray-900 dark:text-gray-100"><?php echo e(\Carbon\Carbon::parse($log->tanggal)->format('d M Y')); ?></div>
                                    <span class="text-[9px] font-bold text-gray-400 dark:text-gray-500 block mt-0.5"><?php echo e(\Carbon\Carbon::parse($log->tanggal)->isoFormat('dddd')); ?></span>
                                </td>
                                ...
                                <td class="px-5 py-4">
                                    <div class="flex items-center gap-2.5">
                                        <div class="w-8 h-8 rounded-full bg-purple-50 dark:bg-purple-950/60 text-purple-600 dark:text-purple-300 border border-purple-200 dark:border-purple-800/60 flex items-center justify-center font-bold text-xs flex-shrink-0 shadow-xs">
                                            <?php echo e(strtoupper(substr($log->application->user->name ?? 'U', 0, 1))); ?>

                                        </div>
                                        <div class="min-w-0">
                                            <p class="text-xs font-black text-gray-900 dark:text-gray-100 truncate"><?php echo e($log->application->user->name ?? 'User Terhapus'); ?></p>
                                            <p class="text-[10px] text-gray-500 dark:text-gray-400 font-semibold truncate"><?php echo e($log->application->position->judul_posisi ?? 'Posisi Terhapus'); ?></p>
                                        </div>
                                    </div>
                                </td>

                                <td class="px-5 py-4">
                                    <div class="text-xs text-gray-800 dark:text-gray-200 whitespace-pre-wrap break-words max-w-xs md:max-w-sm"><?php echo e($log->kegiatan); ?></div>
                                </td>

                                <td class="px-5 py-4 text-center">
                                    <?php if($log->bukti_foto_path): ?>
                                        <a href="<?php echo e(route('storage.access', ['type' => 'logbook', 'filename' => basename($log->bukti_foto_path)])); ?>" target="_blank" class="inline-block relative group" title="Lihat Bukti Foto">
                                            <img src="<?php echo e(route('storage.access', ['type' => 'logbook', 'filename' => basename($log->bukti_foto_path)])); ?>" class="w-10 h-10 rounded-xl object-cover border border-gray-200 dark:border-gray-700 hover:scale-110 transition shadow-xs">
                                ...
                                <td class="px-5 py-4">
                                    <?php if($log->application->pembimbing_lapangan): ?>
                                        <div class="text-xs font-bold text-gray-800 dark:text-gray-200"><?php echo e($log->application->pembimbing_lapangan->name); ?></div>
...
<?php endif; ?>
<?php /**PATH C:\EnvKit\projects\aplikasi-magang\aplikasi-magang\resources\views\admin_instansi\laporan\jurnal_harian.blade.php ENDPATH**/ ?>