                ...
                <div class="relative flex md:flex-row justify-start md:justify-between items-start md:items-center gap-y-3 sm:gap-y-6 gap-x-2 overflow-x-auto md:overflow-visible no-scrollbar pb-2 md:pb-0 -mx-1 px-1">
                    <?php $__currentLoopData = $stages; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $stage): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div class="flex items-center gap-3 md:flex-col md:text-center md:flex-1 relative shrink-0 md:shrink">
                            ...
                            <?php if($index < 6): ?>
                                <div class="hidden md:block absolute top-5 left-1/2 right-[-50%] h-[3px] <?php echo e($currentStage > $index ? 'bg-teal-500 dark:bg-teal-400' : 'bg-gray-200 dark:bg-gray-700'); ?> -z-0"></div>
                            <?php endif; ?>
                            ...
                            <div class="w-10 h-10 rounded-full flex items-center justify-center shrink-0 border-2 font-bold transition duration-300 relative z-10
                                ...
                                <?php if($currentStage > $index): ?>
                                    <i class="fas fa-check text-xs"></i>
                                <?php else: ?>
                                    <i class="fas <?php echo e($stage['icon']); ?> text-xs"></i>
                                <?php endif; ?>
                            </div>

                            
                            <div class="md:mt-2 max-w-[140px] sm:max-w-none md:max-w-[120px] md:mx-auto">
                                <h4 class="text-xs font-black <?php echo e($currentStage == $index ? 'text-teal-600 dark:text-teal-400' : ($currentStage > $index ? 'text-gray-700 dark:text-gray-300' : 'text-gray-400 dark:text-gray-500')); ?>"><?php echo e($stage['name']); ?></h4>
                                <p class="text-[11px] sm:text-[10px] text-gray-400 dark:text-gray-500 font-semibold mt-0.5 leading-tight"><?php echo e($stage['desc']); ?></p>
                            </div>
                        </div>
...
<?php endif; ?>
            ...
            <?php if(isset($daysRemaining) && $daysRemaining >= 0 && $daysRemaining <= 7): ?>
                    ...
                    <div class="flex-grow">
                        <h4 class="text-xs font-extrabold text-blue-800 dark:text-blue-300 uppercase tracking-wider mb-1 flex items-center gap-2">
                            ...
                            <span class="countdown-badge-pulse inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-black bg-rose-600 text-white shadow-xs">
                                ...
                                <?php echo e($daysRemaining == 0 ? 'HARI INI' : $daysRemaining . ' HARI LAGI'); ?>
            ...
            <?php if(empty(Auth::user()->nik) || empty(Auth::user()->asal_instansi)): ?>
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <div class="mt-1 sm:mt-0.5 text-xs text-amber-900 dark:text-amber-200 font-medium">
                                ...
                                <a href="<?php echo e(route('profile.edit')); ?>" class="font-bold underline hover:text-amber-950 dark:hover:text-amber-100 mt-2 sm:mt-0 sm:ml-1 inline-block">Lengkapi Sekarang &rarr;</a>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <?php if($activeApp && in_array($activeApp->status?->value, ['diterima', 'selesai'])): ?>
                ...
                <div class="welcome-banner bg-white dark:bg-gray-800 rounded-2xl sm:rounded-3xl shadow-xs border border-gray-100 dark:border-gray-700 overflow-hidden mb-6">
                        ...
                        <div class="w-full md:w-auto text-center md:text-left animate-fade-in-up">
                            <div class="flex flex-col md:flex-row items-center md:items-start gap-3 sm:gap-4 mb-4">
                                ...
                                <div class="text-center md:text-left">
                                    <span class="inline-flex items-center px-3 py-0.5 rounded-full text-xs font-bold <?php echo e($activeApp->display_status == 'selesai' ? 'bg-blue-50 dark:bg-blue-950/60 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-800/60' : ($activeApp->display_status == 'belum mulai' ? 'bg-indigo-50 dark:bg-indigo-950/60 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800/60' : 'bg-teal-50 dark:bg-teal-950/60 text-teal-700 dark:text-teal-300 border border-teal-200 dark:border-teal-800/60')); ?> mb-1.5">
                                        <span class="w-1.5 h-1.5 rounded-full <?php echo e($activeApp->display_status == 'selesai' ? 'bg-blue-500' : ($activeApp->display_status == 'belum mulai' ? 'bg-indigo-500' : 'bg-teal-500')); ?> mr-1.5 animate-ping-slow"></span>
                                        <?php echo e($activeApp->display_status == 'selesai' ? 'Telah Selesai' : ($activeApp->display_status == 'belum mulai' ? 'Belum Mulai' : 'Sedang Magang Aktif')); ?>

                                    </span>
                                    <h3 class="text-xl sm:text-2xl font-black text-gray-900 dark:text-gray-100">Halo, <?php echo e(Auth::user()->name); ?>! 👋</h3>
                                    <p class="text-xs sm:text-sm text-gray-500 dark:text-gray-400 mt-0.5 font-medium"><?php echo e($activeApp->display_status == 'selesai' ? 'Program magang Anda telah berakhir. Selamat!' : ($activeApp->display_status == 'belum mulai' ? 'Magang Anda akan segera dimulai. Persiapkan diri Anda!' : 'Pastikan mengisi logbook dan absensi setiap hari kerja.')); ?></p>
                                </div>
                            </div>
                            ...
                            <div class="inline-flex flex-col sm:flex-row gap-2 sm:gap-3 text-xs font-bold text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-900 p-2.5 sm:p-3 rounded-2xl border border-gray-200 dark:border-gray-700 w-full sm:w-auto">
                                <div class="flex items-center gap-2 justify-center sm:justify-start">
                                    ...
                                    Masuk: <?php echo e(\Carbon\Carbon::parse($jamKerja->jam_mulai_masuk)->format('H:i')); ?> WIB
                                </div>
                                ...
                                <div class="flex items-center gap-2 justify-center sm:justify-start">
                                    ...
                                    Pulang: <?php echo e(\Carbon\Carbon::parse($jamKerja->jam_mulai_pulang)->format('H:i')); ?> WIB
                                </div>
                            </div>
                        </div>

                        <div class="flex flex-wrap justify-center gap-3 w-full md:w-auto">
                            <?php if($activeApp->display_status == 'selesai'): ?>
                            ...
                            <?php elseif($activeApp->display_status == 'belum mulai'): ?>
                            ...
                            <?php elseif(!$attendanceToday): ?>
                                <div class="grid grid-cols-2 sm:flex sm:flex-row gap-2 sm:gap-3 w-full sm:w-auto">
                                    <form id="form-absen-masuk" action="<?php echo e(route('peserta.absen.masuk')); ?>" method="POST" class="w-full sm:w-auto">
                                        <?php echo csrf_field(); ?>
                             ...
                             <?php elseif($attendanceToday->status == 'hadir' && empty($attendanceToday->clock_out)): ?>
                                <div class="flex flex-col items-center gap-3 w-full sm:w-auto">
                                    <div class="text-xs font-bold text-teal-700 dark:text-teal-300 bg-teal-50 dark:bg-teal-950/60 px-4 py-2 rounded-xl border border-teal-200 dark:border-teal-800/60">
                                        <i class="fas fa-check-circle mr-1"></i> Datang: <?php echo e(\Carbon\Carbon::parse($attendanceToday->clock_in)->format('H:i')); ?>

                                    </div>
                                    <form id="form-absen-pulang" action="<?php echo e(route('peserta.absen.pulang')); ?>" method="POST" class="w-full sm:w-auto">
                                        <?php echo csrf_field(); ?>
                            ...
                            <?php else: ?>
                                <div class="text-center w-full sm:w-auto">
                                    <?php if($attendanceToday->status == 'hadir'): ?>
                                        <div class="px-5 py-4 sm:px-6 bg-emerald-50 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300 rounded-2xl border border-emerald-200 dark:border-emerald-800/60 font-bold flex flex-col items-center shadow-xs text-xs">
                                            ...
                                            <span class="text-[11px] font-normal mt-1 text-emerald-600 dark:text-emerald-400">
                                                ...
                                                <?php echo e($attendanceToday->clock_out ? \Carbon\Carbon::parse($attendanceToday->clock_out)->format('H:i') : '?'); ?>
                                    ...
                                    <?php else: ?>
                                        <div class="px-5 py-3 sm:px-6 min-h-[44px] bg-amber-50 dark:bg-amber-950/60 text-amber-700 dark:text-amber-300 rounded-2xl border border-amber-200 dark:border-amber-800/60 font-bold flex items-center gap-2 shadow-xs text-xs">
                                            <i class="fas fa-info-circle text-amber-500"></i> Status Absen: <?php echo e(ucfirst($attendanceToday->status)); ?>
                    ...
                    <?php echo $__env->make('peserta.dashboard._gps-widget', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
                </div>

                
                <?php if($activeApp && $activeApp->status?->value === 'selesai' && empty($activeApp->saran_peserta)): ?>
                            ...
                            <div class="flex gap-3 sm:gap-4 items-start">
                                ...
                                <div class="space-y-1">
                                    ...
                                    <p class="text-gray-600 dark:text-gray-400 text-xs md:text-sm font-medium leading-relaxed max-w-3xl">
                                        Selamat! Masa magang Anda telah selesai. Mohon berikan saran dan evaluasi konstruktif untuk <strong><?php echo e($activeApp->position->instansi->nama_dinas); ?></strong>. Evaluasi Anda bersifat <strong>anonim</strong> dan wajib diisi sebelum mengunduh Sertifikat & Transkrip Nilai Anda.
                                    </p>
                                </div>
                            </div>
                        </div>

                        <form action="<?php echo e(route('peserta.saran.store', $activeApp->id)); ?>" method="POST" class="mt-4 sm:mt-6 space-y-4">
                            <?php echo csrf_field(); ?>
                            <div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-6">
                    ...
                    <?php echo $__env->make('peserta.dashboard._stats', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>

                    <?php echo $__env->make('peserta.dashboard._absensi-card', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>

                    <?php echo $__env->make('peserta.dashboard._logbook-card', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>

                </div>

                <?php if (isset($component)) { $__componentOriginal9f64f32e90b9102968f2bc548315018c = $component; } ?>
<?php if (isset($attributes)) { $__attributesOriginal9f64f32e90b9102968f2bc548315018c = $attributes; } ?>
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.modal','data' => ['name' => 'modal-izin','focusable' => true]] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['name' => 'modal-izin','focusable' => true]); ?>
                    <div class="p-4 sm:p-6 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100">
                        ...
                        <form action="<?php echo e(route('peserta.absen.izin')); ?>" method="POST" enctype="multipart/form-data">
                            <?php echo csrf_field(); ?>
...
<?php endif; ?>
                        ...
                        <div class="flex gap-3 sm:gap-4 items-start">
                            ...
                            <div>
                                ...
                                <p class="text-teal-100/90 text-xs sm:text-sm mt-1 max-w-2xl font-medium leading-relaxed">
                                    Gunakan fitur <strong>Penempatan Otomatis</strong>! Sistem akan menyalurkan Anda secara otomatis ke instansi yang masih kuota tersedia sesuai dengan kualifikasi jurusan Anda (<?php echo e(Auth::user()->major ?? '-'); ?>).
                                </p>
                            </div>
                        </div>
                        <a href="<?php echo e(route('peserta.apply_automatic.form')); ?>" class="shrink-0 w-full sm:w-auto min-h-[44px] text-center bg-white dark:bg-gray-800 text-teal-800 dark:text-teal-300 hover:bg-teal-50 px-5 sm:px-6 py-3 rounded-xl font-bold shadow-md hover:shadow-lg transition transform hover:-translate-y-0.5 active:scale-95 text-xs uppercase tracking-wider flex items-center justify-center gap-2">
            ...
            <?php echo $__env->make('peserta.dashboard._lamaran-list', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
...
<?php endif; ?>
<?php /**PATH C:\EnvKit\projects\aplikasi-magang\aplikasi-magang\resources\views\peserta\dashboard.blade.php ENDPATH**/ ?>