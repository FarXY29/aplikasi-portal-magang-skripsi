...
<div class="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-6">
        ...
        <?php if(count($kampusData)): ?>
            <div class="relative flex items-center justify-center w-full min-h-[200px] sm:min-h-[260px]" style="height: 260px;">
                <canvas id="kampusChart"
                    class="w-full h-full"
                    data-labels="<?php echo e(json_encode($kampusLabels)); ?>"
                    data-values="<?php echo e(json_encode($kampusData)); ?>">
                </canvas>
            </div>
        <?php else: ?>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 p-4 md:p-6 transition-all duration-300 hover:shadow-md">
        ...
        <?php if(count($instansiChartData)): ?>
            <div class="relative flex items-center justify-center w-full min-h-[200px] sm:min-h-[260px]" style="height: 260px;">
                <canvas id="instansiChart"
                    class="w-full h-full"
                    data-labels="<?php echo e(json_encode($instansiChartLabels)); ?>"
                    data-values="<?php echo e(json_encode($instansiChartData)); ?>">
                </canvas>
            </div>
        <?php else: ?>
            <div class="flex h-[260px] flex-col items-center justify-center text-center">
...
<?php /**PATH C:\EnvKit\projects\aplikasi-magang\aplikasi-magang\resources\views\admin_kota\dashboard\_charts.blade.php ENDPATH**/ ?>